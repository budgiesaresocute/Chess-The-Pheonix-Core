// =====================================
// Phoenix CORE++ Stockfish Cluster (v11.1)
// Multi-engine pool • Race-safe scheduler • Freeze-proof • Production-safe
// =====================================

const MAX_ENGINES = 2;
const MAX_PV = 5;
const CACHE_TTL_MS = 15000;
const INFO_THROTTLE_MS = 10;
...
const STOP_WORDS = new Set([
  "score","nps","time","nodes","hashfull","tbhits",
  "multipv","depth","currmove","currmovenumber","pv"
]);
...
// ================= CLEAN DISPOSE (FIXED MEMORY LEAK) =================
...
function markDead(i) {
  ...
  setTimeout(() => spawnSlot(i), 800); // safe recovery
}
...
function finish(req, moves) {
  if (!req || req.done || req.session !== session) return;
...
function startWatchdog() {
  ...
  watchdogTimer = setInterval(() => {
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      if (s.state === ENGINE_STATE.UCI && t - s.lastTick > 9000) {
        markDead(i);
      }
    }
  }, 2000);
...
self.onmessage = (e) => {
  ...
  if (cmd === "search") startSearch(fen, depth, mpv);
};
